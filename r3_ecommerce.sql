...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetProductDetails` ()   BEGIN
    SELECT p.product_name, p.category_id, s.supplier_name
    FROM products p
...
(76, 'Juan Dela Cruz', 'jdc@gmail.com', 'Japan', '09032323902', '$2y$10$XCMJBh5WRqi0kZzoLrIGW.CdKhe9ccwljznDPyajQOR34kbFNnkfK');
...
--
DELIMITER $$
CREATE TRIGGER `orders_update_trigger` AFTER UPDATE ON `orders` FOR EACH ROW BEGIN
    INSERT INTO audittrail (user_id, transaction_type, transaction_date, details)
    VALUES (NEW.customer_id, 'Order Update', NOW(), CONCAT('Order ID: ', NEW.order_id, ' updated.'));
END
$$
DELIMITER ;
...
(280, 'Canon EosC', '../img/img_648696adb86cf.png', 4, 112, '22090', 'These cameras offer exceptional image quality, cinematic capabilities, and advanced video features.\r\n\r\nFeature large sensors, allowing for high-resolution video capture with wide dynamic range and low noise.'),
